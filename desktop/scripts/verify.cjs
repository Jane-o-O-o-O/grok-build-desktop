...
const required = [
  ...
  "provider-config.cjs", "provider-bridge.cjs", "native-config.cjs", "account-info.cjs", "git-workspace.cjs", "cli-runtime.cjs", "attachment-store.cjs", "scripts/test-providers.cjs", "scripts/test-provider-bridge.cjs", "scripts/test-native-config.cjs", "scripts/test-account-info.cjs", "scripts/test-git-workspace.cjs", "scripts/test-cli-runtime.cjs", "scripts/test-attachments.cjs",
...
for (const file of ["main.cjs", "preload.cjs", "provider-config.cjs", "provider-bridge.cjs", "native-config.cjs", "account-info.cjs", "git-workspace.cjs", "cli-runtime.cjs", "attachment-store.cjs", "renderer/i18n.js", "renderer/locales-native.js", "renderer/app.js", "scripts/serve.cjs"]) {
...
if (packageJson.version !== packageLock.version || packageJson.version !== packageLock.packages?.[""]?.version) {
  throw new Error(`package.json and package-lock.json version mismatch (${packageJson.version} vs ${packageLock.version})`);
}
if (!packageJson.build?.files?.includes("attachment-store.cjs") || packageJson.scripts?.["test:attachments"] !== "node scripts/test-attachments.cjs") {
  throw new Error("Clipboard attachment storage is missing from the packaged desktop build or test scripts");
}
...
for (const id of ["messages", "promptInput", "sendButton", "fileDropOverlay", "threadList", "branchButton", "branchPopover", "branchList", "settingsBackdrop", "settingsSearch", "settingsDoneButton", "settingsPendingStatus", "rawConfigEditor", "accountPopover", "authMenuButton", "providerUrl", "discoverModelsButton", "dockTabs", "dockTabAdd", "dockTabPrev", "dockTabNext", "dockDynamicPanes", "fileTree", "fileFilterInput", "fileCodeView", "fileMetaLang", "fileMetaLines", "fileSaveButton", "sidebarResizer", "inspectorResizer", "agentModeButton", "agentModeLabel", "localeSelect", "runtimeDockStatus", "runtimeDockLabel", "runtimeSetupBackdrop", "macosTipBackdrop", "pickRuntimeBinary", "clearRuntimeBinary", "openRuntimeSetup"]) {
...
for (const feature of ["scheduleStreamingRender", "scheduleSideStreamingRender", "requestAnimationFrame", "openPicker", "picker-popover", "dock-status--workspace", "dock-status--local", "grokLogoShimmer", "assets/grok-mark.png", "discoverProviderModels", "nativeSettingGroups", "config:save-raw", "auth:login", "onAuthEvent", "git:info", "switchGitBranch", "branch-popover", "dock-tabbar", "terminal:create", "onTerminalEvent", "side-task-composer", "data-browser-view", "normalizeBrowserUrl", "refreshActiveBrowserPane", "syncBrowserNavButtons", "updateBrowserTabLabel", "dock-pane--browser", "browser.placeholder", "browser.loadFailed", "disposeBrowserPane", "tidyThreads", "toast--error", "toastRecent", "localizedSlashCommand", "welcome.q1.prompt", "context.empty", "side.emptyTitle", "side.sharedPrompt", "tool.input", "tool.waiting", "message.you", "timeline.submit", "preview.response", "composer.autoWith", "preview.version", "data-i18n-title=\"composer.attach\"", "data-i18n-placeholder=\"palette.placeholder\"", "toggleInspector", "setInspectorOpen", "workspace:list", "attachments:save-clipboard-image", "attachments:validate-paths", "saveClipboardImage", "pathForFile", "handlePromptPaste", "bindAttachmentDropZone", "is-file-dragging", "providers:discover", "providers:probe-all", "providers:refresh", "providers:set-enabled", "setProviderModelsEnabled", "data-toggle-provider", "data-provider-model-toggle", "data-provider-card", "data-provider-enabled-status", "providerModelDrafts", "changedProviderDrafts", "updateProviderDraftUI", "applySettingsChanges", "settings-open", "animation-play-state:paused", "backdrop-filter:none", "onProviderEvent", "startSessionUpdateBridge", "tool_call_update", "toolMessageMarkup", "thinking-block", "permission_requested", "runtime:models", "hydrateRuntimeModels", "settings-search-hit", "settingsSearchCatalog", "integration-detail-modal", "openIntegrationDetail", "slash-popover", "slashCommands", "file-explorer", "file-tree", "pane-resizer", "bindPaneResizer", "applyPaneWidths", "tool-steps", "toolGroupMarkup", "ensureActiveAssistant", "updateTurnProgress", "stream-caret", "agent-mode-picker", "openAgentModePicker", "permissionMode", "buildCliArgs", "streaming-json", "platform-darwin", "GrokI18n", "applyLocale", "localeSelect", "locales-native.js", "nativeSettingTitle", "runtime:pick-binary", "runtime:clear-binary", "pickRuntimeBinary", "clearRuntimeBinary", "openRuntimeSetup", "maybeShowMacosGatekeeperTip", "RUNTIME_NOT_FOUND", "customBinary", "guide-modal", "macosTipBackdrop", "detectFileLanguage", "fileMetaLang", "file-code__empty-icon", "workspace:write", "writeWorkspaceFile", "saveWorkspaceFile", "fileCodeEditor", "bindFileTreeOnce"]) {
  if (!`${html}\n${js}\n${css}\n${backend}\n${i18n}`.includes(feature)) throw new Error(`Desktop interaction missing: ${feature}`);
}
